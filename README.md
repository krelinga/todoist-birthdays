...
| `TZ` | No | `America/Chicago` | Timezone used to decide when "today" ticks over, when `RUN_AT` fires, and to render log timestamps. |
...
| `DETAILED_LOGGING` | No | `false` | If truthy (`1`/`true`/`yes`/`on`), logs the outcome of every person considered each run at DEBUG level (outside notice window, already sent, task created), not just the run summary and errors. |
